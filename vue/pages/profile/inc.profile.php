...
// On fait la requête en utilisant la Foreign Key qui contient l'ID de la personne connectée.
...
if (!$profil) {
    // Si aucun profil n'est trouvé, on crée un profil par défaut
    $insert = $bdd->prepare('INSERT INTO profil (utilisateur_id, nom, prénom, date_naissance, telephone, adresse, bio)
VALUES (:utilisateur_id, "", "", NULL, "", "", "")');
    $insert->execute(['utilisateur_id' => $env_id]);

    // On recharge les informations du profil après l'insertion
    $requete->execute(['valeur_id' => $env_id]);
    $profil = $requete->fetch(PDO::FETCH_ASSOC);
}
